export default function Header() {
    return (
        <header className="it-header-height z-index-2">
            <div id="header-sticky" className="it-header-wrap p-relative">
                {/* Header Top */}
                <div className="it-header-top-area it-header-top-ptb black-bg">
                    <div className="container container-1650">
                        <div className="row align-items-center">
                            <div className="col-xxl-7 col-xl-8 col-lg-7 col-md-6 col-sm-6">
                                <div className="it-header-top-list-box">
                                    <ul>
                                        <li>
                                            <span>
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M15.8333 0.833984H4.16667C3.062 0.835308 2.00296 1.27472 1.22185 2.05583C0.440735 2.83695 0.00132321 3.89599 0 5.00065L0 15.0007C0.00132321 16.1053 0.440735 17.1644 1.22185 17.9455C2.00296 18.7266 3.062 19.166 4.16667 19.1673H15.8333C16.938 19.166 17.997 18.7266 18.7782 17.9455C19.5593 17.1644 19.9987 16.1053 20 15.0007V5.00065C19.9987 3.89599 19.5593 2.83695 18.7782 2.05583C17.997 1.27472 16.938 0.835308 15.8333 0.833984ZM4.16667 2.50065H15.8333C16.3323 2.50163 16.8196 2.65191 17.2325 2.93215C17.6453 3.2124 17.9649 3.60977 18.15 4.07315L11.7683 10.4557C11.2987 10.9234 10.6628 11.186 10 11.186C9.33715 11.186 8.70131 10.9234 8.23167 10.4557L1.85 4.07315C2.03512 3.60977 2.35468 3.2124 2.76754 2.93215C3.1804 2.65191 3.66768 2.50163 4.16667 2.50065ZM15.8333 17.5007H4.16667C3.50363 17.5007 2.86774 17.2373 2.3989 16.7684C1.93006 16.2996 1.66667 15.6637 1.66667 15.0007V6.25065L7.05333 11.634C7.83552 12.4142 8.89521 12.8524 10 12.8524C11.1048 12.8524 12.1645 12.4142 12.9467 11.634L18.3333 6.25065V15.0007C18.3333 15.6637 18.0699 16.2996 17.6011 16.7684C17.1323 17.2373 16.4964 17.5007 15.8333 17.5007Z" fill="#E03B3B" />
                                                </svg>
                                                <a className="border-line-white" href="mailto:info@naql-afsh.com">info@naql-afsh.com</a>
                                            </span>
                                        </li>
                                        <li className="d-none d-lg-inline-block">
                                            <span>
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <g clipPath="url(#clip0_loc)">
                                                        <path d="M9.99984 5C9.34057 5 8.6961 5.1955 8.14794 5.56177C7.59977 5.92804 7.17253 6.44863 6.92024 7.05772C6.66795 7.66681 6.60194 8.33703 6.73055 8.98363C6.85917 9.63024 7.17664 10.2242 7.64282 10.6904C8.10899 11.1565 8.70293 11.474 9.34954 11.6026C9.99614 11.7312 10.6664 11.6652 11.2755 11.4129C11.8845 11.1606 12.4051 10.7334 12.7714 10.1852C13.1377 9.63707 13.3332 8.9926 13.3332 8.33333C13.3332 7.44928 12.982 6.60143 12.3569 5.97631C11.7317 5.35119 10.8839 5 9.99984 5ZM9.99984 10C9.6702 10 9.34797 9.90225 9.07389 9.71912C8.79981 9.53598 8.58619 9.27568 8.46004 8.97114C8.33389 8.6666 8.30089 8.33149 8.3652 8.00818C8.42951 7.68488 8.58824 7.38791 8.82133 7.15482C9.05442 6.92174 9.35139 6.763 9.67469 6.69869C9.99799 6.63438 10.3331 6.66739 10.6376 6.79353C10.9422 6.91968 11.2025 7.1333 11.3856 7.40738C11.5688 7.68147 11.6665 8.0037 11.6665 8.33333C11.6665 8.77536 11.4909 9.19928 11.1784 9.51184C10.8658 9.82441 10.4419 10 9.99984 10Z" fill="#E03B3B" />
                                                        <path d="M9.99977 19.9997C9.29806 20.0033 8.6057 19.8387 7.98066 19.5198C7.35562 19.2008 6.8161 18.7367 6.40727 18.1664C3.23144 13.7855 1.62061 10.4922 1.62061 8.37721C1.62061 6.15492 2.50341 4.02365 4.07481 2.45225C5.64621 0.880849 7.77748 -0.00195312 9.99977 -0.00195312C12.2221 -0.00195312 14.3533 0.880849 15.9247 2.45225C17.4961 4.02365 18.3789 6.15492 18.3789 8.37721C18.3789 10.4922 16.7681 13.7855 13.5923 18.1664C13.1834 18.7367 12.6439 19.2008 12.0189 19.5198C11.3938 19.8387 10.7015 20.0033 9.99977 19.9997ZM9.99977 1.81721C8.26012 1.8192 6.59229 2.51115 5.36217 3.74127C4.13204 4.97139 3.44009 6.63923 3.43811 8.37888C3.43811 10.0539 5.01561 13.1514 7.87894 17.1005C8.12202 17.4354 8.44091 17.7079 8.80953 17.8958C9.17815 18.0837 9.58602 18.1816 9.99977 18.1816C10.4135 18.1816 10.8214 18.0837 11.19 17.8958C11.5586 17.7079 11.8775 17.4354 12.1206 17.1005C14.9839 13.1514 16.5614 10.0539 16.5614 8.37888C16.5595 6.63923 15.8675 4.97139 14.6374 3.74127C13.4073 2.51115 11.7394 1.8192 9.99977 1.81721Z" fill="#E03B3B" />
                                                    </g>
                                                    <defs>
                                                        <clipPath id="clip0_loc">
                                                            <rect width="20" height="20" fill="white" />
                                                        </clipPath>
                                                    </defs>
                                                </svg>
                                                <a className="border-line-white" target="_blank" href="https://maps.google.com">القاهرة، مصر</a>
                                            </span>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <div className="col-xxl-5 col-xl-4 col-lg-5 col-md-6 col-sm-6 d-none d-sm-block">
                                <div className="d-flex justify-content-end align-items-center">
                                    <div className="it-header-top-contact d-none d-xxl-inline-block">
                                        <a href="#about">من نحن</a>
                                        <span>/</span>
                                        <a href="#service">خدماتنا</a>
                                        <span>/</span>
                                        <a href="#contact">تواصل معنا</a>
                                    </div>
                                    <div className="it-header-top-social-box text-end">
                                        <a href="#" aria-label="فيسبوك">
                                            <svg width="12" height="16" viewBox="0 0 12 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path fillRule="evenodd" clipRule="evenodd" d="M1.82727 6.83333C1.14284 6.83333 1 6.96763 1 7.61111V8.77778C1 9.42126 1.14284 9.55556 1.82727 9.55556H3.48182V14.2222C3.48182 14.8657 3.62466 15 4.30909 15H5.96364C6.64807 15 6.79091 14.8657 6.79091 14.2222V9.55556H8.64871C9.1678 9.55556 9.30155 9.4607 9.44416 8.99145L9.7987 7.82478C10.043 7.02095 9.89246 6.83333 9.00326 6.83333H6.79091V4.88889C6.79091 4.45933 7.16129 4.11111 7.61818 4.11111H9.97273C10.6572 4.11111 10.8 3.97681 10.8 3.33333V1.77778C10.8 1.1343 10.6572 1 9.97273 1H7.61818C5.33373 1 3.48182 2.74111 3.48182 4.88889V6.83333H1.82727Z" stroke="currentcolor" strokeWidth="1.5" strokeLinejoin="round" />
                                            </svg>
                                        </a>
                                        <a href="#" aria-label="واتساب">
                                            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z" fill="currentcolor" />
                                                <path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.79 23.528l4.631-1.467A11.943 11.943 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-2.281 0-4.388-.732-6.106-1.976l-.438-.292-2.755.873.921-2.68-.32-.485A9.948 9.948 0 012 12C2 6.486 6.486 2 12 2s10 4.486 10 10-4.486 10-10 10z" fill="currentcolor" />
                                            </svg>
                                        </a>
                                        <a href="#" aria-label="انستجرام">
                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <path d="M12.0586 4.94727C12.6109 4.94727 13.0586 4.49955 13.0586 3.94727C13.0586 3.39498 12.6109 2.94727 12.0586 2.94727V4.94727ZM12.0496 2.94727C11.4973 2.94727 11.0496 3.39498 11.0496 3.94727C11.0496 4.49955 11.4973 4.94727 12.0496 4.94727V2.94727ZM8 14C6.32181 14 5.16377 13.9979 4.2928 13.8808C3.45059 13.7675 3.02803 13.5636 2.73223 13.2678L1.31802 14.682C2.04735 15.4113 2.96231 15.7199 4.0263 15.8629C5.06152 16.0021 6.37835 16 8 16V14ZM0 8C0 9.62165 -0.00212373 10.9385 0.137058 11.9737C0.280107 13.0377 0.588687 13.9526 1.31802 14.682L2.73223 13.2678C2.43644 12.972 2.23246 12.5494 2.11922 11.7072C2.00212 10.8362 2 9.67819 2 8H0ZM14 8C14 9.67819 13.9979 10.8362 13.8808 11.7072C13.7675 12.5494 13.5636 12.972 13.2678 13.2678L14.682 14.682C15.4113 13.9526 15.7199 13.0377 15.8629 11.9737C16.0021 10.9385 16 9.62165 16 8H14ZM8 16C9.62165 16 10.9385 16.0021 11.9737 15.8629C13.0377 15.7199 13.9526 15.4113 14.682 14.682L13.2678 13.2678C12.972 13.5636 12.5494 13.7675 11.7072 13.8808C10.8362 13.9979 9.67819 14 8 14V16ZM8 2C9.67819 2 10.8362 2.00212 11.7072 2.11922C12.5494 2.23246 12.972 2.43644 13.2678 2.73223L14.682 1.31802C13.9526 0.588687 13.0377 0.280107 11.9737 0.137058C10.9385 -0.00212373 9.62165 0 8 0V2ZM16 8C16 6.37835 16.0021 5.06152 15.8629 4.0263C15.7199 2.96231 15.4113 2.04735 14.682 1.31802L13.2678 2.73223C13.5636 3.02803 13.7675 3.45059 13.8808 4.2928C13.9979 5.16377 14 6.32181 14 8H16ZM8 0C6.37835 0 5.06152 -0.00212373 4.0263 0.137058C2.96231 0.280107 2.04735 0.588687 1.31802 1.31802L2.73223 2.73223C3.02803 2.43644 3.45059 2.23246 4.2928 2.11922C5.16377 2.00212 6.32181 2 8 2V0ZM2 8C2 6.32181 2.00212 5.16377 2.11922 4.2928C2.23246 3.45059 2.43644 3.02803 2.73223 2.73223L1.31802 1.31802C0.588687 2.04735 0.280107 2.96231 0.137058 4.0263C-0.00212373 5.06152 0 6.37835 0 8H2ZM10.3171 8.00134C10.3171 9.28031 9.28031 10.3171 8.00134 10.3171V12.3171C10.3849 12.3171 12.3171 10.3849 12.3171 8.00134H10.3171ZM8.00134 10.3171C6.72236 10.3171 5.68555 9.28031 5.68555 8.00134H3.68555C3.68555 10.3849 5.61779 12.3171 8.00134 12.3171V10.3171ZM5.68555 8.00134C5.68555 6.72236 6.72236 5.68555 8.00134 5.68555V3.68555C5.61779 3.68555 3.68555 5.61779 3.68555 8.00134H5.68555ZM8.00134 5.68555C9.28031 5.68555 10.3171 6.72236 10.3171 8.00134H12.3171C12.3171 5.61779 10.3849 3.68555 8.00134 3.68555V5.68555ZM12.0586 2.94727H12.0496V4.94727H12.0586V2.94727Z" fill="currentcolor" />
                                            </svg>
                                        </a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                {/* Main Header */}
                <div className="it-header-area it-header-style-4 white-bg it-header-ptb">
                    <div className="container container-1650">
                        <div className="p-relative">
                            <div className="row align-items-center">
                                <div className="col-xxl-2 col-xl-2 col-lg-6 col-md-6 col-6">
                                    <div className="it-header-logo-red">
                                        <a href="/"><img src="/assets/img/logo/logo-red.png" alt="نقل العفش" /></a>
                                    </div>
                                </div>
                                <div className="col-xxl-7 col-xl-7 d-none d-xl-block">
                                    <div className="it-header-menu it-header-dropdown">
                                        <nav className="it-menu-content">
                                            <ul className="it-onepage-menu">
                                                <li><a className="active" href="#slider">الرئيسية</a></li>
                                                <li><a href="#about">من نحن</a></li>
                                                <li><a href="#service">خدماتنا</a></li>
                                                <li><a href="#choose">لماذا نحن</a></li>
                                                <li><a href="#testimonial">آراء العملاء</a></li>
                                                <li><a href="#project">أعمالنا</a></li>
                                                <li><a href="#team">فريقنا</a></li>
                                            </ul>
                                        </nav>
                                    </div>
                                </div>
                                <div className="col-xxl-3 col-xl-3 col-lg-6 col-md-6 col-6">
                                    <div className="it-header-right-action d-flex justify-content-end align-items-center">
                                        <a href="#contact" className="it-btn-orange d-none d-md-flex">
                                            <span>
                                                <span className="text-1">احجز الآن</span>
                                                <span className="text-2">احجز الآن</span>
                                            </span>
                                            <i>
                                                <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M10.0035 3.90804L1.41153 12.5L0 11.0885L8.59097 2.49651H1.01922V0.5H12V11.4808H10.0035V3.90804Z" fill="white" />
                                                </svg>
                                                <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M10.0035 3.90804L1.41153 12.5L0 11.0885L8.59097 2.49651H1.01922V0.5H12V11.4808H10.0035V3.90804Z" fill="white" />
                                                </svg>
                                            </i>
                                        </a>
                                        <div className="it-header-bar d-xl-none">
                                            <button className="it-menu-bar">
                                                <span>
                                                    <svg width="24" height="20" viewBox="0 0 24 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path fillRule="evenodd" clipRule="evenodd" d="M10 18.3333C10 17.4128 10.7462 16.6667 11.6667 16.6667H21.6667C22.5872 16.6667 23.3333 17.4128 23.3333 18.3333C23.3333 19.2538 22.5872 20 21.6667 20H11.6667C10.7462 20 10 19.2538 10 18.3333ZM0 1.66667C0 0.746183 0.746183 0 1.66667 0H21.6667C22.5872 0 23.3333 0.746183 23.3333 1.66667C23.3333 2.58713 22.5872 3.33333 21.6667 3.33333H1.66667C0.746183 3.33333 0 2.58713 0 1.66667ZM0 10C0 9.07953 0.746183 8.33333 1.66667 8.33333H21.6667C22.5872 8.33333 23.3333 9.07953 23.3333 10C23.3333 10.9205 22.5872 11.6667 21.6667 11.6667H1.66667C0.746183 11.6667 0 10.9205 0 10Z" fill="currentcolor" />
                                                    </svg>
                                                </span>
                                            </button>
                                        </div>
                                        <div className="it-header-top-list ml-40 d-none d-xxl-flex align-items-center">
                                            <div className="it-header-top-list-icon">
                                                <span>
                                                    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                        <path d="M9.74999 0.749245C9.74999 0.550332 9.82901 0.359567 9.96966 0.218915C10.1103 0.0782622 10.3011 -0.000755399 10.5 -0.000755399C12.4884 0.0014283 14.3948 0.792306 15.8009 2.19836C17.2069 3.60441 17.9978 5.51079 18 7.49924C18 7.69816 17.921 7.88892 17.7803 8.02958C17.6397 8.17023 17.4489 8.24924 17.25 8.24924C17.0511 8.24924 16.8603 8.17023 16.7197 8.02958C16.579 7.88892 16.5 7.69816 16.5 7.49924C16.4982 5.90849 15.8655 4.38341 14.7407 3.25858C13.6158 2.13375 12.0907 1.50103 10.5 1.49924C10.3011 1.49924 10.1103 1.42023 9.96966 1.27957C9.82901 1.13892 9.74999 0.948157 9.74999 0.749245ZM10.5 4.49924C11.2956 4.49924 12.0587 4.81531 12.6213 5.37792C13.1839 5.94053 13.5 6.7036 13.5 7.49924C13.5 7.69816 13.579 7.88892 13.7197 8.02958C13.8603 8.17023 14.0511 8.24924 14.25 8.24924C14.4489 8.24924 14.6397 8.17023 14.7803 8.02958C14.921 7.88892 15 7.69816 15 7.49924C14.9988 6.30614 14.5243 5.16224 13.6807 4.31858C12.837 3.47492 11.6931 3.00044 10.5 2.99924C10.3011 2.99924 10.1103 3.07826 9.96966 3.21891C9.82901 3.35957 9.74999 3.55033 9.74999 3.74924C9.74999 3.94816 9.82901 4.13892 9.96966 4.27957C10.1103 4.42023 10.3011 4.49924 10.5 4.49924ZM17.3197 12.5535C17.7544 12.9893 17.9984 13.5797 17.9984 14.1952C17.9984 14.8108 17.7544 15.4012 17.3197 15.837L16.6372 16.6237C10.4947 22.5045 -4.45276 7.56074 1.33724 1.39874L2.19974 0.648745C2.63607 0.226253 3.22119 -0.00746859 3.82852 -0.00185427C4.43585 0.00376005 5.01655 0.248258 5.44499 0.678745C5.46824 0.701995 6.85799 2.50724 6.85799 2.50724C7.27036 2.94047 7.49992 3.51594 7.49894 4.11405C7.49796 4.71215 7.26653 5.28687 6.85274 5.71874L5.98424 6.81074C6.46488 7.97858 7.17154 9.03993 8.06362 9.93382C8.9557 10.8277 10.0156 11.5365 11.1825 12.0195L12.2812 11.1457C12.7132 10.7323 13.2878 10.5011 13.8857 10.5003C14.4836 10.4994 15.0589 10.729 15.492 11.1412C15.492 11.1412 17.2965 12.5302 17.3197 12.5535ZM16.2877 13.644C16.2877 13.644 14.493 12.2632 14.4697 12.24C14.3152 12.0868 14.1065 12.0008 13.8889 12.0008C13.6713 12.0008 13.4625 12.0868 13.308 12.24C13.2877 12.261 11.775 13.4662 11.775 13.4662C11.673 13.5474 11.5517 13.6006 11.423 13.6206C11.2942 13.6406 11.1625 13.6267 11.0407 13.5802C9.52908 13.0174 8.15604 12.1363 7.01462 10.9966C5.87319 9.85681 4.99004 8.48507 4.42499 6.97425C4.37489 6.85084 4.35855 6.71631 4.37767 6.5845C4.39678 6.45269 4.45065 6.32834 4.53374 6.22425C4.53374 6.22425 5.73899 4.71074 5.75924 4.69124C5.91244 4.53673 5.9984 4.32796 5.9984 4.11037C5.9984 3.89278 5.91244 3.68401 5.75924 3.52949C5.73599 3.50699 4.35524 1.71074 4.35524 1.71074C4.19841 1.57012 3.99374 1.49482 3.78317 1.50025C3.57261 1.50569 3.3721 1.59147 3.22274 1.73999L2.36024 2.48999C-1.87126 7.57799 11.082 19.8127 15.5407 15.5992L16.224 14.8117C16.3841 14.6634 16.4803 14.4587 16.4922 14.2408C16.5041 14.0229 16.4308 13.8088 16.2877 13.644Z" fill="currentcolor" />
                                                    </svg>
                                                </span>
                                            </div>
                                            <div className="it-header-top-list-text">
                                                <span>اتصل بنا الآن</span>
                                                <a className="border-line-black" href="tel:+201234567890">+20 123 456 7890</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </header>
    );
}
