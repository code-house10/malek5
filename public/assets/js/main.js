(function ($) {
	...
	// for range (jQuery UI slider - only run if jQuery UI is loaded)
	...
	if ($('.it-split-text').length > 0) {
		...
		st.each(function (index, el) {
			...
			const hasArabic = /[\u0600-\u06FF]/.test($(el).text());
			// For Arabic, do NOT split by chars as it breaks ligatures
			const splitType = hasArabic ? "lines,words" : "lines,words,chars";
